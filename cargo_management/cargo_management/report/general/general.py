...
def get_gl_entries(filters, accounting_dimensions):
    ...
    if filters.get("include_default_book_entries"):
        filters["company_fb"] = frappe.get_cached_value("Company", filters.get("company"), "default_finance_book")
    ...
    # تمرير opration_currency من الفلاتر
    filters["opration_currency"] = filters.get("opration_currency")
    ...
    # استعلام مع فلترة على أساس opration_currency
    opr_condition = ""
    if filters.get("opration_currency"):
        opr_condition = "AND transaction_currency = %(opration_currency)s"

    gl_entries_opr = frappe.db.sql(
        """
        select
            name as gl_entry, posting_date, account, party_type, party,
            voucher_type, voucher_subtype, voucher_no, {dimension_fields}
            ...
            against, is_opening, creation {select_fields}
        from `tabGL Entry`
        where company=%(company)s {opr_condition} {conditions}
        {order_by_statement}
        """.format(
            dimension_fields=dimension_fields,
            transaction_currency_fields=transaction_currency_fields,
            ...
            conditions=get_conditions(filters),
            order_by_statement=order_by_statement,
            opr_condition=opr_condition,
        ),
        filters,
        as_dict=1,
    )

    # تحقق من الاستعلام وقيمة opration_currency
    frappe.msgprint("Current Filters: " + str(filters))
    frappe.msgprint("Constructed SQL Query: " + frappe.db.last_query)
    if filters.get("opration_currency"):
        frappe.msgprint("Selected Operation Currency: " + str(filters["opration_currency"]))
    else:
        frappe.msgprint("No Operation Currency selected.")

    # استرجاع النتائج بناءً على العملة المحددة
    if filters.get("opration_currency"):
        return gl_entries_opr
