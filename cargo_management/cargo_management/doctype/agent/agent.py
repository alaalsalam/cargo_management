...
class Agent(TransactionBase):
	...
	# 	if cust_master_name == "Agent Name":
	...
	# 	if frappe.db.get_value("Agent", self.agent_name) and not frappe.flags.in_import:
	# 		count = frappe.db.sql(
	# 			"""select ifnull(MAX(CAST(SUBSTRING_INDEX(name, ' ', -1) AS UNSIGNED)), 0) from tabAgent
	# 			 where name like %s""",
	# 			f"%{self.agent_name} - %",
	# 			as_list=1,
	# 		)[0][0]
	...
	# 		new_agent_name = f"{self.agent_name} - {cstr(count)}"
	...
	def get_customer_group_details(self):
		doc = frappe.get_doc("Agent Group", self.agent_group)
		self.accounts = []
		self.credit_limits = []
	...
	def check_customer_group_change(self):
			frappe.flags.agent_group_changed = False

			if not self.get("__islocal"):
				if self.agent_group != frappe.db.get_value("Agent", self.name, "agent_group"):
					frappe.flags.agent_group_changed = True
	...
	def update_customer_groups(self):
		...
		if frappe.flags.agent_group_changed:
			update_linked_doctypes(
				"Agent", self.name, "Agent Group", self.agent_group, ignore_doctypes
			)
	...
	def link_lead_address_and_contact(self):
			if self.lead_name:
				...
				for row in linked_contacts_and_addresses:
					...
					if not linked_doc.has_link("Agent", self.name):
						linked_doc.append("links", dict(link_doctype="Agent", link_name=self.name))
	...
	def validate_name_with_customer_group(self):
		if frappe.db.exists("Agent Group", self.name):
			frappe.throw(
				_(
					"A Agent Group exists with same name please change the Agent name or rename the Agent Group"
				),
				frappe.NameError,
			)
	...
	def on_trash(self):
		...
		delete_contact_and_address("Agent", self.name)
		if self.lead_name:
			frappe.db.sql("update `tabLead` set status='Interested' where name=%s", self.lead_name)

	# def after_rename(self, olddn, newdn, merge=False):
	# 		if frappe.defaults.get_global_default("cust_master_name") == "Agent Name":
	# 			self.db_set("agent_name", newdn)
...
def make_quotation(source_name, target_doc=None):
	...
	target_doc = get_mapped_doc(
		"Agent",
		source_name,
		{"Agent": {"doctype": "Quotation", "field_map": {"name": "party_name"}}},
		target_doc,
		set_missing_values,
	)

	target_doc.quotation_to = "Agent"
	target_doc.run_method("set_missing_values")
	target_doc.run_method("set_other_charges")
	target_doc.run_method("calculate_taxes_and_totals")

	price_list, currency = frappe.db.get_value(
		"Agent", {"name": source_name}, ["default_price_list", "default_currency"]
	)
	if price_list:
...
def make_contact(args, is_primary_contact=1):
	...
	party_type = args.agent_type if args.doctype == "Agent" else args.supplier_type
	party_name_key = "agent_name" if args.doctype == "Agent" else "supplier_name"
...
def get_customer_outstanding(customer, company, ignore_outstanding_sales_order=False, cost_center=None):
	...
	outstanding_based_on_gle = frappe.db.sql(
		...
		from `tabGL Entry` where party_type = 'Agent'
