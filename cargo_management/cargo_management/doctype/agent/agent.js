...
frappe.ui.form.on("Agent", {
	setup: function (frm) {
		frm.make_methods = {
			Quotation: () =>
				frappe.model.open_mapped_doc({
					method: "cargo_management.shipment_management.doctype.agent.agent.make_quotation",
					frm: cur_frm,
				}),
			Opportunity: () =>
				frappe.model.open_mapped_doc({
					method: "cargo_management.shipment_management.doctype.agent.agent.make_opportunity",
					frm: cur_frm,
				}),
		};

		frm.add_fetch("lead_name", "company_name", "agent_name");
		frm.add_fetch("default_sales_partner", "commission_rate", "default_commission_rate");
		frm.set_query("agent_group", { is_group: 0 });
		...
		frm.set_query("customer_primary_contact", function (doc) {
			return {
				query: "cargo_management.shipment_management.doctype.agent.agent.get_customer_primary_contact",
				filters: {
					customer: doc.name,
				},
			};
	...
	is_internal_agent: function (frm) {
		if (frm.doc.is_internal_agent == 1) {
	...
	refresh: function (frm) {
		...
		} else {check_customer_group_change
