...
class ParcelPriceRule(Document):
	pass
	...
	def apply(self, doc):
		...
		if self.calculate_based_on == "Actual Weight":
			# value = doc.base_net_total
			value = doc.total_actual_weight
			by_value = True

		elif self.calculate_based_on == "Volumetric Weight":
			# value = doc.total_net_weight
			value = doc.total_volumetric_weight
			by_value = True
		...
		doc.shipping_amount = shipping_amount
		# doc.shipping_amount = shipping_amount
